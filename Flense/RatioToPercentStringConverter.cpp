#include "pch.h"

#include "RatioToPercentStringConverter.h"
#if __has_include("RatioToPercentStringConverter.g.cpp")
#include "RatioToPercentStringConverter.g.cpp"
#endif

#include <format>

using namespace winrt::Windows::UI::Xaml::Interop;

namespace winrt::Flense::implementation
{
    winrt::Windows::Foundation::IInspectable RatioToPercentStringConverter::Convert(
        const winrt::Windows::Foundation::IInspectable& value, const TypeName& /* targetType */,
        const winrt::Windows::Foundation::IInspectable& /* parameter */, const hstring& /* language */)
    {
        const float ratio = value.as<float>();
        return winrt::box_value(winrt::hstring(std::format(L"{:.1f}%", ratio * 100.0f)));
    }

    winrt::Windows::Foundation::IInspectable RatioToPercentStringConverter::ConvertBack(
        const winrt::Windows::Foundation::IInspectable& /* value */, const TypeName& /* targetType */,
        const winrt::Windows::Foundation::IInspectable& /* parameter */, const hstring& /* language */)
    {
        // One-way binding only
        throw hresult_not_implemented();
    }
} // namespace winrt::Flense::implementation
